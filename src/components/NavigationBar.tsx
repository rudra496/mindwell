...
const navItems = [
  ...
  { id: "psychologists-page", label: "Psychologists", icon: Stethoscope },
  { id: "crisis-emergency-help", label: "Crisis & Emergency Help", icon: Phone },
  { id: "free-services-bangladesh", label: "Free Services in Bangladesh 🇧🇩", icon: MapPin },
      ...
      <nav className="hidden lg:block bg-white/95 backdrop-blur-md shadow-md sticky top-0 z-50 border-b border-teal-200" role="navigation" aria-label="Main navigation">
        <div className="container mx-auto px-4 py-3">
          <div className="flex items-center justify-between gap-4">
            <button onClick={() => handleNavClick("home")} className="flex items-center gap-2" aria-label="MindWell Home">
        ...
        <div className="container mx-auto px-3 py-3">
          ...
          {mobileMenuOpen && (
            <div className="mt-3 pb-2 space-y-1 border-t border-slate-200 pt-2 bg-white/95">
              {navItems.map((item) => (
                <Button key={item.id} variant="ghost" onClick={() => handleNavClick(item.id)} className="w-full justify-start text-gray-700 hover:text-teal-700 hover:bg-teal-50 min-h-[44px]">
