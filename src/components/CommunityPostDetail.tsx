...
export function CommunityPostDetail({
  ...
  const isAuthor = localPost?.userId === currentUid;
  const hasLikedPost = useMemo(
    () => Array.isArray(localPost?.likedBy) && !!currentUid && localPost.likedBy.includes(currentUid),
    [localPost, currentUid]
  );

  const rootComments = useMemo(
    () => replies.filter((reply) => !reply.parentReplyId),
    [replies]
  );
  ...
  const handleSubmitComment = async () => {
    ...
    try {
      setNewComment("");
      await addReply(post.id, newComment.trim(), null);
      onPostUpdate?.();
    } catch {
  ...
  const handleSubmitNestedReply = async () => {
    ...
    try {
      const content = newNestedReply.trim();
      setNewNestedReply("");
      setReplyingToReplyId(null);
      await addReply(post.id, content, replyingToReplyId);
      onPostUpdate?.();
    } catch {
  ...
  const handleLikePost = async () => {
    ...
    const authed = await requireAuth();
    if (!authed) {
      setError("You must sign in to like posts.");
      return;
    }

    const optimisticLiked = !hasLikedPost;
    const optimisticLikedBy = optimisticLiked
      ? [...(localPost.likedBy || []), currentUid]
      : (localPost.likedBy || []).filter((id: string) => id !== currentUid);

    const previous = localPost;
    setLocalPost({ ...localPost, likedBy: optimisticLikedBy, likes: optimisticLikedBy.length });

    try {
      const result = await togglePostLike(post.id);
      const syncedLikedBy = result.liked
        ? [...(previous.likedBy || []).filter((id: string) => id !== currentUid), currentUid]
        : (previous.likedBy || []).filter((id: string) => id !== currentUid);

      setLocalPost((prev: any) => ({
        ...prev,
        likedBy: syncedLikedBy,
        likes: result.likes,
      }));
      onPostUpdate?.();
    } catch {
  ...
  const handleLikeReplyClick = async (replyId: string) => {
    ...
    const hasLiked = (target.likedBy || []).includes(currentUid);

    setReplies((prev) =>
      prev.map((r) => {
        if (r.id !== replyId) return r;
        const nextLikedBy = hasLiked
          ? (r.likedBy || []).filter((id) => id !== currentUid)
          : [...(r.likedBy || []), currentUid];
        return {
          ...r,
          likedBy: nextLikedBy,
          ...
          <div className="space-y-6">
              ...
              <CardContent className="pt-6 space-y-4">
                ...
                {editingPost ? (
                  <div className="space-y-3">
                    ...
                    <div className="flex gap-2">
                      ...
                      <Button variant="outline" size="sm" onClick={() => setEditingPost(false)}>
                        Cancel
                      </Button>
                    </div>
                  </div>
                  ...
                  <>
                    ...
                    <p className="text-sm text-muted-foreground">
                      {localPost.displayName || "Anonymous"} • {safeDate(localPost.createdAt)}
                    </p>
                ...
                <div className="flex gap-3 pt-3 border-t flex-wrap">
                  ...
                  {isAuthor && !editingPost && (
                    <>
                      <Button variant="outline" size="sm" onClick={beginEditPost}>
                        <Edit className="h-4 w-4 mr-2" />
                        Edit
                      </Button>

                      <Button variant="destructive" size="sm" onClick={removePost}>
            ...
            {crisis && (
              <Alert className="border-red-500 bg-red-50">
                ...
                <AlertDescription className="text-red-900">
                  If you are in danger call emergency services now. US: 988 • Text HELLO to 741741
                </AlertDescription>
              </Alert>
            )}
            ...
            {isLoadingReplies ? (
              ...
              rootComments.map((reply) => {
                ...
                const hasLikedReply = !!currentUid && (reply.likedBy || []).includes(currentUid);
                  ...
                  <Card key={reply.id}>
                    <CardContent className="pt-6 space-y-3">
                      ...
                      <div className="flex gap-2 items-center pt-2 border-t flex-wrap">
                        <Button size="sm" variant={hasLikedReply ? "default" : "ghost"} onClick={() => handleLikeReplyClick(reply.id)}>
                          <ThumbsUp className="h-3 w-3 mr-1" />
                          {reply.likes || 0}
                        </Button>

                        <Button size="sm" variant="ghost" onClick={() => setReplyingToReplyId(reply.id)}>
                        ...
                        {reply.userId === currentUid && (
                          <>
                            <Button variant="outline" size="sm" onClick={() => beginEdit(reply)}>
                              Edit
                            </Button>
                            <Button variant="destructive" size="sm" onClick={() => removeReply(reply.id)}>
                              Delete
                            </Button>
                          </>
                        )}
                      ...
                      {replyingToReplyId === reply.id && (
                        <div className="mt-2 pl-3 border-l-2 space-y-2">
                          ...
                          />
                          <div className="flex gap-2">
                            <Button size="sm" onClick={handleSubmitNestedReply} disabled={!newNestedReply.trim() || isSubmittingNestedReply}>
                              {isSubmittingNestedReply ? "Posting..." : "Post Reply"}
                            </Button>
                            <Button variant="outline" size="sm" onClick={() => setReplyingToReplyId(null)}>
                              Cancel
                            </Button>
                          </div>
                        </div>
                      ...
                      {nestedReplies.length > 0 && (
                        <div className="pl-4 border-l-2 space-y-2">
                          {nestedReplies.map((nested) => {
                            const nestedLiked = !!currentUid && (nested.likedBy || []).includes(currentUid);
                            return (
                              <div key={nested.id} className="rounded-md border p-3 space-y-2">
                                ...
                                <div className="flex items-center gap-2">
                                  <Button
                                    size="sm"
                                    variant={nestedLiked ? "default" : "ghost"}
                                    onClick={() => handleLikeReplyClick(nested.id)}
                                  >
                                    <ThumbsUp className="h-3 w-3 mr-1" />
                                    {nested.likes || 0}
                                  </Button>
                                  <span className="text-xs text-muted-foreground">{safeDate(nested.createdAt)}</span>
                                  {nested.userId === currentUid && (
                                    <Button variant="destructive" size="sm" onClick={() => removeReply(nested.id)}>
                                      Delete
                                    </Button>
                                  )}
                                </div>
