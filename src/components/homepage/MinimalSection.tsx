...
export function MinimalSection({
  ...
  return (
    ...
    >
      <div
        className={`relative min-h-[240px] md:min-h-[320px] group bg-gradient-to-br ${bgGradient} transition-all duration-300 ${!stockPhotoUrl ? "border-b border-white/50 dark:border-slate-700 shadow-inner bg-[length:200%_200%] bg-[position:0%_50%]" : ""}`}
      >
        {stockPhotoUrl && (
          <div className="absolute inset-0 overflow-hidden opacity-[0.42]">
            <Image
              src={stockPhotoUrl}
              alt={stockPhotoAlt}
        ...
        <div className="relative z-10 p-6 sm:p-8 flex items-center gap-4">
          <div className="w-14 h-14 sm:w-16 sm:h-16 rounded-full bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm flex items-center justify-center shadow-lg">
      ...
      <CardContent className="p-6 sm:p-8 pt-6 bg-white dark:bg-slate-800 text-xl [&_button]:text-lg [&_button]:px-7 [&_button]:py-3 [&_button]:h-auto">
        {children}
      </CardContent>
    </Card>
  )
